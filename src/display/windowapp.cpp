...
WindowedApplication::WindowedApplication(uint32_t width, uint32_t height, const std::vector<const char*>& validation_layers)
    ...
    , window(Window(context, width, height))
...
static void render_loop(IDisplayable** displayers, Camera& camera, Display& display, Dispatcher& dispatcher, UiControlPanel& control_panel)
{
...
void WindowedApplication::begin(const Scene& scene, Camera& camera)
{
    ...
    RasteriseDisplayer rasterise_displayer(display, dispatcher, scene, display.get_extent(), surface_format.format, choose_depth_format(device));
    rasterise_displayer.set_camera(dispatcher, camera);

    PathTraceDisplayer path_trace_displayer(display, dispatcher, scene, display.get_extent());
    path_trace_displayer.set_camera(dispatcher, camera);
